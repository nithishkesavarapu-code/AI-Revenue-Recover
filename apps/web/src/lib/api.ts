...
function loadLocalApiEnvironment() {
  if (process.env.API_URL && process.env.API_AUTH_TOKEN && process.env.DEMO_MODE && process.env.DEMO_ACCESS_TOKEN) return;

  const envPath = path.resolve(process.cwd(), "../..", ".env");
  if (!existsSync(envPath)) return;

  for (const line of readFileSync(envPath, "utf8").split(/\r?\n/)) {
    const match = line.match(/^(API_URL|API_AUTH_TOKEN|DEMO_MODE|DEMO_ACCESS_TOKEN)=(.*)$/);
